...
AddrSpace::AddrSpace()
{
    ...
    for (int i = 0; i < NumPhysPages; i++) {
        ...
        bzero(&(kernel->machine->mainMemory[pageTable[i].physicalPage * PageSize]), MemorySize); 
        DEBUG(dbgAddr, "DEBUG Virtual memory: Physical page of virtual page "<< i << ": " << pageTable[i].physicalPage << "\n");
    }
...
// how big is address space?
    ...
    DEBUG(dbgAddr, "DEBUG: Number of physical pages: " << NumPhysPages << "\n");
    
    if (numPages > kernel->gPhysPageBitMap->NumClear())
    {
        DEBUG(dbgAddr, "Not enough free memory in virtual memory bitmap");
        delete executable;
        numPages = 0;
        kernel->addrLock->V();
        return;
    ...
    DEBUG(dbgAddr, "DEBUG: Translate virtual to physical page successfully\n");

// then, copy in the code and data segments into memory
// Note: this code assumes that virtual address = physical address
    if (noffH.code.size > 0) {
        for (int i=0; i<numPages; i++){
            DEBUG(dbgAddr, "Copy code to virtual memory page " << i);
            executable->ReadAt(
    ...
    if (noffH.initData.size > 0) {
        for (int i=0; i<numPages; i++){
            DEBUG(dbgAddr, "Copy data segment to virtual memory page " << i);
            executable->ReadAt(
    ...
    if (noffH.readonlyData.size > 0) {
        for (int i=0; i<numPages; i++){
            executable->ReadAt(
            &(kernel->machine->mainMemory[noffH.readonlyData.virtualAddr + pageTable[i].physicalPage*PageSize]),
                PageSize, noffH.readonlyData.inFileAddr+(i*PageSize));
        }
    }
#endif
    DEBUG(dbgAddr, "DEBUG: Copy code and data to memory successfully");
...
// how big is address space?
    ...
    for (int i = 0; i < numPages; i++) {
        ...
        ,PageSize);
        DEBUG(dbgAddr, "Physicial page of virtual page " << i << ": " << pageTable[i].physicalPage); 
    }
    if (noffH.code.size > 0)
    {
        for(int i = 0; i < numPages ; i++)
...
AddrSpace::Execute() 
{
    ...
    DEBUG(dbgAddr, "Load page table registers and states successfully");

...
{
    if (threadName != NULL){
        DEBUG(dbgThread, "Create thread with name " << threadName);
    }
    name = threadName;
    stackTop = NULL;
    stack = NULL;
...
void Thread::FreeSpace(){
    ...
    this->space = NULL; //Scheduler will delete thread latr, what we need is just free thread's address space
}
//----------------------------------------------------------------------
// Thread::Finish
...
Thread::Finish ()
{
    DEBUG(dbgThread, "Finishing thread with PID "<< this->processID << "\n");
    DEBUG(dbgThread, "Current thread "<< kernel->currentThread->processID << "\n");
